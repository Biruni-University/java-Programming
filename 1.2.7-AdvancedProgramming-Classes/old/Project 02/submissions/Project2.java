...
public class Project2 {
    ...
    public static void main(String Args[])  {        //useing an array with index in conjunction with main HashSet would be better but this project won't allow it
        ...
        int corP,incorP,corPTmp=0,incorPTmp=0,choice;

        while(true)    {       //the game
            //System.out.println(i);
            choice = getRandomElement(valid);
            first=  choice /1000;
            ...
            for(int j=0;j<9999;j++)  {         //traverse the "valid"                
                int firstall=  j /1000;
                int secondall= j/100%10;
                int thirdall=  j/10%10;
                int forthall=  j%10;
                ...
                if(corP!=corPTmp || incorP!=incorPTmp)  {    //remove the nummber that have diffrent mastermindLike points than user given ones
                    valid.remove(j);
                }
                incorPTmp=0;corPTmp=0;      //this took me 3 hours to find AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
            }
        }
        scanner.close();
    }
